...
namespace simplicity
{
	namespace opengl
	{
		...
		TEST_F(SimpleOpenGLPickerTest, createPickEventShape)
		{
			...
			NiceMock<MockTreeNode> mockNode0;
			NiceMock<MockShape> mockShape0;
			NiceMock<MockTreeNode> mockNode1;
			NiceMock<MockShape> mockShape1;

			// Dictate correct behaviour.
			// //////////////////////////////////////////////////
			ON_CALL(*mockScene, getTree()).WillByDefault(ReturnRef(mockTree));
			ON_CALL(mockTree, get(0)).WillByDefault(ReturnRef(mockNode0));
			ON_CALL(mockNode0, getComponent()).WillByDefault(Return(&mockShape0));
			ON_CALL(mockTree, get(1)).WillByDefault(ReturnRef(mockNode1));
			ON_CALL(mockNode1, getComponent()).WillByDefault(Return(&mockShape1));
			...
			// Perform test
			// //////////////////////////////////////////////////
			PickEvent pickEvent = createPickEventPublic(2);

			// Verify test results.
			// //////////////////////////////////////////////////
			ASSERT_EQ(2, pickEvent.getHitCount());
			ASSERT_EQ(&mockNode0, pickEvent.getHit(0).node);
			ASSERT_EQ(NULL, pickEvent.getHit(0).primitive.get());
			ASSERT_EQ(&mockNode1, pickEvent.getHit(1).node);
			ASSERT_EQ(NULL, pickEvent.getHit(1).primitive.get());
		}
		...
		TEST_F(SimpleOpenGLPickerTest, createPickEventVertexGroup)
		{
			...
			NiceMock<MockTree<> > mockTree;
			NiceMock<MockTreeNode> mockNode0;
			NiceMock<MockVertexGroup> mockParentVertexGroup0;
			shared_ptr<MockVertexGroup> mockChildVertexGroup0(new NiceMock<MockVertexGroup>);
			NiceMock<MockTreeNode> mockNode1;
			NiceMock<MockVertexGroup> mockParentVertexGroup1;
			...
			ON_CALL(*mockScene, getTree()).WillByDefault(ReturnRef(mockTree));\
			ON_CALL(mockTree, get(0)).WillByDefault(ReturnRef(mockNode0));
			ON_CALL(mockNode0, getComponent()).WillByDefault(Return(&mockParentVertexGroup0));
			ON_CALL(mockParentVertexGroup0, createFaceSubsetVG(20)).WillByDefault(Return(mockChildVertexGroup0));
			ON_CALL(mockTree, get(1)).WillByDefault(ReturnRef(mockNode1));
			ON_CALL(mockNode1, getComponent()).WillByDefault(Return(&mockParentVertexGroup1));
