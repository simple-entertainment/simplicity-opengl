...
namespace simplicity
{
	namespace opengl
	{
		/**
		 * <p>
		 * A viewpoint within a <code>SceneGraph</code> rendered by a JOGL rendering environment. This implementation
		 * uses only simple camera techniques and properties.
		 * </p>
		 *
		 * <p>
		 * The viewing frustum is the shape that contains all components of the <code>SceneGraph</code> displayed when
		 * viewing through a <code>SimpleJOGLCamera</code>. The following diagram shows the 'side' and 'front' views of
		 * a viewing frustum.
		 * </p>
		 *
		 * <pre>
		...
		class SimpleOpenGLCamera : public simplicity::Camera
		{
			public:
				...
				void setFarClippingDistance(const float farClippingDistance);

				void setFrameAspectRatio(const float frameAspectRatio);

				void setFrameHeight(const float frameHeight);

				void setFrameWidth(const float frameWidth);

				void setFrameX(const float frameX);

				void setFrameY(const float frameY);
			...
			private:
				...
				/**
				 * <p>
				 * The distance from the eye past which components of the <code>SceneGraph</code> will be clipped (not
				 * drawn).
				 * </p>
				 */
				...
				/**
				 * <p>
				 * The distance from the eye before which components of the <code>SceneGraph</code> will be clipped (not
				 * drawn).
				 * </p>
				 */
				...
				/**
				 * <p>
				 * The inverted absolute transformation for the <code>Node</code> of this <code>Camera</code>, or a zero
